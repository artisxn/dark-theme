...
class On extends Command
{
     ...
     * @var string
     */
    protected $signature = 'dark-theme:on';
    ...
    protected function addDarkThemeClass()
    {
        ...
        if (! Str::contains($template, 'dark-theme')) {
            $pattern = '#<html[^>]+class\s*=\s*"#';
            $template = preg_replace($pattern, '$0dark-theme ', $template);

            File::put($path, $template);

            $this->info("\nAdded dark-theme class to Nova's layout template:");
        } else {
            $this->info("\ndark-theme class already present in Nova's layout template:");
        }
